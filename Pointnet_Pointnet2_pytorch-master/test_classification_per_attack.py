...
def test(model, loader, num_class=2, vote_num=1, perturb_channels=[0,1,2], perturb_eps=0):
    mean_correct = []
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        if perturb_eps != 0:
            points = perturbation_attack(points, perturb_channels, perturb_eps)
...
def main(args):
    ...
    file_handler = logging.FileHandler('%s/eval_perturbation_attack.txt' % experiment_dir)
    ...
    with torch.no_grad():
        ...
        for i, eps in enumerate(eps_values):
            ...
            instance_acc, class_acc = test(classifier.eval(), testDataLoader, vote_num=args.num_votes, num_class=num_class, perturb_channels=perturb_channels, perturb_eps=eps)
        ...
        # filename =  f"/per_attack_comparison_epoch_{args.epoch}_npoint_{args.num_point}_bsize_{args.batch_size}"
        # if args.dropout:
        #     filename = filename + "_dropout"
        # if args.shift:
        #     filename = filename + "_shift"
        filename = filename + ".csv"
        df.to_csv(experiment_dir + filename, index=False)
