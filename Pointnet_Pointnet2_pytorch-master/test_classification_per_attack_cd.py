...
under adversarial ONLY perturbation attack
"""
...
def test(model, loader, num_class=2, vote_num=1, is_confusion_matrix=False):
    ...
    if is_confusion_matrix:
        # assuming car = positive = label 0
        target_ls = np.array(target_ls)
        pred_ls = np.array(pred_ls)

        TP = np.sum((target_ls == 0) & (pred_ls == 0))
        FN = np.sum((target_ls == 0) & (pred_ls != 0))
        FP = np.sum((target_ls != 0) & (pred_ls == 0))
        TN = np.sum((target_ls != 0) & (pred_ls != 0))
        print("TP:", TP, "FN", FN, "FP", FP, "TN", TN)

    return cd_ls, target_ls, pred_ls


def test_PointGuard(cls_model, scorer_model, loader, num_class=2, vote_num=1, is_confusion_matrix=False):
...
def main(args):
    ...
    test_dataset = ModelNetDataLoader_clean_per_inj(root=data_path, args=args, split='test', process_data=False, per_prob=0.9, inject_prob=0)
    ...
    with torch.no_grad():
        ...
        for _ in range(num_runs):
            #   Acc vs Chamfer Distance
            # ==================================================
            is_confusion_matrix = True
            if args.is_PointGuard:
                cd_ls, target_ls, pred_ls = test_PointGuard(classifier, scorer, testDataLoader, vote_num=args.num_votes, num_class=num_class, is_confusion_matrix=is_confusion_matrix)
            else:
                cd_ls, target_ls, pred_ls = test(classifier, testDataLoader, vote_num=args.num_votes, num_class=num_class, is_confusion_matrix=is_confusion_matrix)
        ...
        # class_acc_series = pd.concat(all_runs_acc, axis=1).mean(axis=1).reindex(results.index)
        # class_acc_series = class_acc_series.interpolate(method='linear').ffill().bfill()
        # cd_upper = [x.left for x in results.index]

        # out_df = pd.DataFrame({
        #     'cd_upper': cd_upper,
        #     'class_acc': class_acc_series.values
        # })

        # # save the data
        # filename =  f"/per_attack_comparison_channel_{args.channels_per}_eps_{args.eps_per}"
