"""
train a discriminator to do binary cls between 
real point cloud (1) and adversarial point cloud (0)
"""
...
def parse_args():
    ...
    parser.add_argument('--num_category', default=2, type=int, choices=[2],  help='training on clean and adversarial')
...
def test(model, loader, num_class=2):
    ...
    for j, (points_aug, target, _) in tqdm(enumerate(loader), total=len(loader)):
        ...
        points, scores = torch.split(points_aug, [4,1], dim=2)    # points (B, N, 4); scores (B,N,1)
        target = (scores == 1).all(dim=1).squeeze(-1).long()       # target (B,);  0 if attacked, 1 if clean
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points_aug, _, _) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            points = points.transpose(2, 1)             # (B, 4, N)
        ...
        with torch.no_grad():
            instance_acc = test(classifier, testDataLoader, num_class=2)
