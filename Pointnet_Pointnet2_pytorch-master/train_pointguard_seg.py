...
def test(model, loader):
    ...
    for j, (points_aug, _, _, _) in tqdm(enumerate(loader), total=len(loader)):   
...
def main(args):
    ...
    data_path = '/content/drive/MyDrive/THESIS_dataset/mmw/MyS3DIS_seg'
    NUM_CLASSES = 4

    train_dataset = S3DISDataset_mix(split='train', data_root=data_path, num_point=1024, sample_rate=1.0, transform=None, samples_per_frame=4, num_classes=NUM_CLASSES,
                                     per_prob=0.4, inj_prob=0.4,
                                     per_channels=[0,1,2,3], per_eps=2,
                                     inj_npoint=160, inj_clutter_size=20)
    test_dataset = S3DISDataset_mix(split='test', data_root=data_path, num_point=1024, sample_rate=1.0, transform=None, samples_per_frame=4, num_classes=NUM_CLASSES,
                                     per_prob=0.4, inj_prob=0.4,
                                     per_channels=[0,1,2,3], per_eps=2,
                                     inj_npoint=160, inj_clutter_size=20)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        scheduler.step()
        for batch_id, (points_aug, _, _, _) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
